...
with open(scripts_dir / f'distill.sh', 'w') as f1:
    ...
    for scene_no in range(1, 7):
        for split in ['a', 'b']:
            f1.write(f'sbatch -p ripl-gpu -c1 -C 2080ti -o {logs_dir}/distill_{scene_no}{split}.out --wrap "singularity exec --nv --bind {slurm_project_dir}:{local_project_dir} --bind {slurm_data_dir}:{slurm_data_dir} {sif_path} bash {local_project_dir}/{scripts_dir}/{scene_no}{split}.sh"\n\n')
